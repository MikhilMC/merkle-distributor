...
describe("MerkleDistributorERC721V1.sol", () => {
  ...
  let MerkleDistributorERC721V1,
    merkleDistributorERC721V1,
    merkleDistributorERC721V1Address;
  ...
  beforeEach(async () => {
    ...
    MerkleDistributorERC721V1 = await ethers.getContractFactory(
      "MerkleDistributorERC721V1"
    );
    merkleDistributorERC721V1 = await MerkleDistributorERC721V1.deploy(
      "Sample NFT",
      "SNFT",
      root
    );
    await merkleDistributorERC721V1.deployed();
    merkleDistributorERC721V1Address = await merkleDistributorERC721V1.address;
  });

  describe("Correct setup", () => {
    it("Should have correct name", async () => {
      const name = await merkleDistributorERC721V1.name();
      expect(name).to.equal("Sample NFT");
    });

    it("Should have correct symbol", async () => {
      const symbol = await merkleDistributorERC721V1.symbol();
      expect(symbol).to.equal("SNFT");
    });

    it("Should have correct merkle root", async () => {
      const merkleRoot = await merkleDistributorERC721V1.root();
  ...
  describe("Correct destribution of NFTs", () => {
    it("Should distribute NFTs correctly", async () => {
      ...
      for (let index = 0; index < airdropData.length; index++) {
        await expect(
          merkleDistributorERC721V1.ownerOf(airdropData[index].tokenId)
        ).to.be.revertedWith("ERC721: invalid token ID");

        proof = tree.getHexProof(leaves[index]);
        await merkleDistributorERC721V1.redeem(
          airdropData[index].address,
          airdropData[index].tokenId,
          proof
        );

        nftOwner = await merkleDistributorERC721V1.ownerOf(
          airdropData[index].tokenId
        );
    ...
    it("Should revert if proof is invalid", async () => {
      ...
      await expect(
        merkleDistributorERC721V1.redeem(
